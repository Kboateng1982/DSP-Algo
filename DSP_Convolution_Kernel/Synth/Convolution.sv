...
// Pipeline registers for the Low-Pass Filter 
(* altera_attribute = {" -name SYNCHRONIZER_IDENTIFICATION OFF "} *) reg [DATA_WIDTH-1:0] Pipe_12_my_tap, Pipe_23_my_tap, Pipe_34_my_tap;
...
begin
		if (!reset_n) 
		    ...
		    begin 
				Pipe_12_my_tap <= my_tap[ADDR_WIDTH_BITS_H_N];
				Pipe_23_my_tap <= Pipe_12_my_tap;
				Pipe_34_my_tap <= Pipe_23_my_tap; 
			end 
end

// Pipeline registers for delayed inputs (delay)
(* altera_attribute = {" -name SYNCHRONIZER_IDENTIFICATION OFF "} *) reg [DATA_WIDTH-1:0] Pipe_12_delay, Pipe_23_delay, Pipe_34_delay;
...
begin
		if (!reset_n) 
		    ...
		    begin 
				Pipe_12_delay <= delay[ADDR_WIDTH_BITS_H_N];
				Pipe_23_delay <= Pipe_12_delay;
				Pipe_34_delay <= Pipe_23_delay; 
			end 
end
 ...
 begin
        if (!reset_n) 
            ...
            conv_done_flag<=0;
				for (w=0; w<ROM_DEPTH_H_N; w++)
					begin
                        prod[w]<= Pipe_34_my_tap * Pipe_34_delay;
                    end 
            end
end 
...
// Pipeline registers for the Addition 
(* altera_attribute = {" -name SYNCHRONIZER_IDENTIFICATION OFF "} *) reg [ACCUM_MSB:0] Pipe_12_my_temp, Pipe_23_my_temp, Pipe_34_my_temp;
...
begin
		if (!reset_n) 
		    ...
		    begin 
				Pipe_12_my_temp <= my_temp[ADDR_WIDTH_BITS_H_N];
				Pipe_23_my_temp <= Pipe_12_my_temp;
				Pipe_34_my_temp <= Pipe_23_my_temp; 
			end 
end
...
begin
		...
		//begin
			my_output_fir_tdata += Pipe_34_my_temp;
		//end
end 
